			...
			if(lineTokens[lineTokens.size() - 2].type == LTT::FromFile) {
				...
				for(int j = 0; j < lineTokens.size(); j++) {
					if(vars.find(lineTokens[j].val) != vars.end()) {
						if(vars[lineTokens[j].val] == LTT::TypeString) {
							ret.string = true;
						}
						if(vars[lineTokens[j].val] == LTT::TypeFloating) {
							ret.floating = true;
						}
						if(vars[lineTokens[j].val] == LTT::TypeInteger) {
							ret.integer = true;
						}
						if(vars[lineTokens[j].val] == LTT::TypeChar) {
							ret.character = true;
						}
						if(vars[lineTokens[j].val] == LTT::TypeBool) {
							ret.boolean = true;
						}
					}
				}
